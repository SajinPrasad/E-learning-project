...
const CommentList = ({ courseId, ws, setParentComment, maxNestingLevel }) => {
  ...
  const fetchProfilePictureAndAddComment = async (newComment) => {
    try {
      const fetchedProfilePicture = await getProfilePictureService(newComment.user_id);
      const completeComment = fetchedProfilePicture
        ? { ...newComment, user_profile_picture: fetchedProfilePicture.profile_picture_url }
        : newComment;
  ...
  const formatCommentDate = (dateString) => {
    ...
    return isToday
      ? commentDate.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })
  ...
  const renderComments = (commentsToRender, level = 0) =>
    commentsToRender.map((comment, index) => (
      <div key={`${comment.id}-${index}`} style={{ marginLeft: `${Math.min(level, 5) * 16}px` }} className="mt-2">
        <div className={`rounded-lg border ${level === 0 ? 'bg-white' : 'bg-gray-50'} p-3 shadow-sm`}>
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-grow">
              <div className="flex items-center justify-between">
                <h5 className="text-xs font-semibold text-gray-800">{comment.user_fullname}</h5>
          ...
          <div className="flex gap-5">
            <p onClick={() => setParentComment(comment)} className="w-fit cursor-pointer text-xs text-gray-400 hover:text-gray-600">
              Reply
            </p>
            {comment.replay_count > 0 && (
              <i onClick={() => handleFetchingReplayComments(comment.id)} className="cursor-pointer text-xs font-semibold">
